...
(){
    ...
    if [[ ${+commands[brew]} == 1 ]] ; then
        ...
        do
            [[ -d ${dir}/bin/brew ]] && export PATH="${prefix}/bin:${prefix}/sbin${PATH:+:${PATH}}" && break
        done
    fi
    [[ ${+commands[brew]} == 1 ]] && eval "$(brew shellenv)"
    prefix="${HOME}/.local"
    if [[ -d ${prefix} ]] ; then
        export INFOPATH="${prefix}/share/info${INFOPATH:+:${INFOPATH}}"
        export MANPATH="${prefix}/share/man${MANPATH:+:${MANPATH}}"
    ...
    for command in python2 python3
    do
        if [[ ${+commands[${command}]} == 1 ]] ; then
            prefix="$(${command} -m site --user-base 2> /dev/null)"
            [[ -n ${prefix} && ${PATH} != *${prefix}/bin* ]] && export PATH="${prefix}/bin${PATH:+:${PATH}}"
        fi
    done
    ...
    if [[ ${+commands[goenv]} == 1 ]] ; then
        ...
        for dir in "${HOME}/.goenv" /usr/local/goenv
        do
    ...
    if [[ ${+commands[nodenv]} == 1 ]] ; then
        for dir in "${HOME}/.nodenv" /usr/local/nodenv
        do
    ...
    if [[ ${+commands[phpenv]} == 1 ]] ; then
        for dir in "${HOME}/.phpenv" /usr/local/phpenv
        do
    ...
    if [[ ${+commands[pyenv]} == 1 ]] ; then
        for dir in "${HOME}/.pyenv" /usr/local/pyenv
        do
    ...
    if [[ ${+commands[rbenv]} == 1 ]] ; then
        for dir in "${HOME}/.rbenv" /usr/local/rbenv
        do
    ...
    if [[ ${+commands[swiftenv]} == 1 ]] ; then
        for dir in "${HOME}/.swiftenv" /usr/local/swiftenv
        do
    ...
    if [[ -n ${HOMEBREW_PREFIX} ]] ; then
        for dir in "${HOME}/.local/google-cloud-sdk" "${HOME}/google-cloud-sdk" /usr/local/google-cloud-sdk "${HOMEBREW_PREFIX}/Caskroom/google-cloud-sdk/latest/google-cloud-sdk"
        do
            [[ -f ${dir}/path.zsh.inc ]] && . "${dir}/path.zsh.inc" && break
        done
        for dir in "${HOME}/.local/google-cloud-sdk" "${HOME}/google-cloud-sdk" /usr/local/google-cloud-sdk "${HOMEBREW_PREFIX}/Caskroom/google-cloud-sdk/latest/google-cloud-sdk" /usr/share/google-cloud-sdk "${HOME}/snap/google-cloud-sdk/current" /snap/google-cloud-sdk/current
        do
            [[ -f ${dir}/completion.zsh.inc ]] && . "${dir}/completion.zsh.inc" && break
        done
    else
        for dir in "${HOME}/.local/google-cloud-sdk" "${HOME}/google-cloud-sdk" /usr/local/google-cloud-sdk
        do
            [[ -f ${dir}/path.zsh.inc ]] && . "${dir}/path.zsh.inc" && break
        done
        for dir in "${HOME}/.local/google-cloud-sdk" "${HOME}/google-cloud-sdk" /usr/local/google-cloud-sdk /usr/share/google-cloud-sdk "${HOME}/snap/google-cloud-sdk/current" /snap/google-cloud-sdk/current
        do
            [[ -f ${dir}/completion.zsh.inc ]] && . "${dir}/completion.zsh.inc" && break
        done
    fi
